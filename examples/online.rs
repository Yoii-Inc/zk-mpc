...
async fn main() {
    ...
    Net::simulate(net, (), move |_, _| async move {
        ...
        let sum_r0 = MFr::from_add_shared(r0).reveal().await;
        let sum_r1 = MFr::from_add_shared(r1).reveal().await;
        let sum_r2 = MFr::from_add_shared(r2).reveal().await;
        ...
        match zksnark {
            ZkSnark::Groth16 => {}
            ZkSnark::Marlin => {
                let srs = LocalMarlin::universal_setup(10000, 50, 100, common_rng)
                    .expect("Failed to setup");

                // commitmnet phase
                // Pedersen commitment
                //// commom parameter
                let params = <Fr as LocalOrMPC<Fr>>::PedersenComScheme::setup(common_rng).unwrap();
                ...
                for x in shared_input.clone() {
                    //// input(parent)
                    let x_parent: Fr = x.clone().reveal().await;
                ...
                mpc_input.set_public_input(common_rng, None);
                mpc_input.set_private_input(None);
                mpc_input.generate_input(common_rng);
                ...
                inputs.push(peculiar_a_commitment.reveal().await.x);
                inputs.push(peculiar_a_commitment.reveal().await.y);
                inputs.push(peculiar_b_commitment.reveal().await.x);
                inputs.push(peculiar_b_commitment.reveal().await.y);

                assert!(LocalMarlin::verify(&index_vk, &inputs, &proof, rng)
